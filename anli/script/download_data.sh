...
if [[ -f anli_v${ANLI_VERSION}/R1/train.jsonl ]] &&
	...
	[[ -f anli_v${ANLI_VERSION}/R3/test.jsonl ]]; then
	echo "ANLI checked."
else
...
# Check data OCNLI:
cd ${DIR_TMP}/data
if [[ -f ocnli/train.50k.json ]] &&
	[[ -f ocnli/dev.json ]] &&
	[[ -f ocnli/test.json ]]; then
	echo "OCNLI checked."
else
	echo "Some OCNLI files are not ready. Please remove the \"ocnli\" directory and run download.sh again."
	ALL_DATA_CHECKED=false
fi

if [[ ${ALL_DATA_CHECKED} == true ]]; then
