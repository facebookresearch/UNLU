...
#
# Train with scrambled input based on our two policies.
# Provide loss for current data
# Add loss to make the distribution of scrambled data to be uniform
...
def get_args():
    ...
    parser.add_argument("--scrambled_train_data", type=str, help="The scrambled data")
    ...
    parser.add_argument("--entropy_lambda", default=1.0, type=float)

    parser.add_argument(
        "--margin", default=2, type=float, help="margin loss",
    )
...
def train(local_rank, args):
    ...
    ## scrambled data
    scrambled_train_data_str = args.scrambled_train_data
    scrambled_train_data_weights_str = args.train_weights

    scrambled_train_data_name = []
    scrambled_train_data_path = []
    scrambled_train_data_list = []
    scrambled_train_data_weights = []
    scrambled_train_data_named_path = scrambled_train_data_str.split(",")

    scrambled_weights_str = (
        scrambled_train_data_weights_str.split(",")
        if scrambled_train_data_weights_str is not None
        else None
    )

    for named_path in scrambled_train_data_named_path:
        ind = named_path.find(":")
        name = named_path[:ind]
        ...
        scrambled_train_data_name.append(name)
        scrambled_train_data_path.append(path)

        scrambled_train_data_list.append(d_list)

    if scrambled_weights_str is not None:
        for weights in scrambled_weights_str:
            scrambled_train_data_weights.append(float(weights))
    else:
        for i in range(len(scrambled_train_data_list)):
            scrambled_train_data_weights.append(1)

    ## make sure total scrambled data = real data
    assert len(scrambled_train_data_list) == len(train_data_list)
    ...
    ## scrambled
    scrambled_training_list = []
    for i in range(len(scrambled_train_data_list)):
        print("Build Scrambled Training Data ...")
        scrambled_train_d_list = scrambled_train_data_list[i]
        scrambled_train_d_name = scrambled_train_data_name[i]
        scrambled_train_d_weight = scrambled_train_data_weights[i]
        scrambled_cur_train_list = sample_data_list(
            scrambled_train_d_list, scrambled_train_d_weight
        )  # change later  # we can apply different sample strategy here.
        print(
            f"Data Name:{scrambled_train_d_name}; Weight: {scrambled_train_d_weight}; "
            f"Original Size: {len(scrambled_train_d_list)}; Sampled Size: {len(scrambled_cur_train_list)}"
        )
        scrambled_training_list.extend(scrambled_cur_train_list)
    ...
    if not args.cpu:
        print(f"Setting device rank {args.local_rank}")
    ...
    for epoch in tqdm(
        ...
        ## scrambling build
        scramble_training_list = []
        for i in range(len(scrambled_train_data_list)):
            print("Build scrambled_Training Data ...")
            scrambled_train_d_list = scrambled_train_data_list[i]
            scrambled_train_d_name = scrambled_train_data_name[i]
            scrambled_train_d_weight = scrambled_train_data_weights[i]
            scrambled_cur_train_list = sample_data_list(
        ...
        random.shuffle(scrambled_training_list)
        scrambled_train_dataset = NLIDataset(scrambled_training_list, data_transformer)

        scrambled_train_sampler = SequentialSampler(scrambled_train_dataset)
        if not args.cpu and not args.single_gpu:
            print("Use distributed sampler.")
            scrambled_train_sampler = DistributedSampler(
                scrambled_train_dataset, args.world_size, args.global_rank, shuffle=True
            )

        scrambled_train_dataloader = DataLoader(
            dataset=scrambled_train_dataset,
            batch_size=batch_size_per_gpu_train,
            shuffle=False,  #
            num_workers=0,
            pin_memory=True,
            sampler=scrambled_train_sampler,
            collate_fn=BaseBatchBuilder(batching_schema),
        )  #
        # scrambling training build finished.
        ...
        for forward_step, (batch, scrambled_batch) in enumerate(
            zip(train_dataloader, scrambled_train_dataloader)
        ):
            model.train()
            ...
            ## scrambling loss
            scrambled_batch = move_to_device(scrambled_batch, local_rank)
            if args.model_class_name in ["distilbert", "bart-large"]:
                scrambled_outputs = model(
                    scrambled_batch["input_ids"],
                    attention_mask=scrambled_batch["attention_mask"],
                    labels=scrambled_batch["y"],
                )
            else:
                scrambled_outputs = model(
                    scrambled_batch["input_ids"],
                    attention_mask=scrambled_batch["attention_mask"],
                    token_type_ids=scrambled_batch["token_type_ids"],
                    labels=scrambled_batch["y"],
                )
            scrambled_loss, scrambled_logits = scrambled_outputs[:2]
            scrambled_loss_val = scrambled_loss.item()

            entropy = F.softmax(scrambled_logits, dim=1) * F.log_softmax(
                scrambled_logits, dim=1
            )
            entropy = -1.0 * entropy.sum()

            loss += args.entropy_lambda * entropy
            # entropy_val = entropy.item()

            # mm = torch.nn.MarginRankingLoss(margin=args.margin)
            # loss = mm(
            #     loss.unsqueeze(0),
            #     scrambled_loss.unsqueeze(0),
            #     torch.ones_like(loss.unsqueeze(0)),
            # )

            total_loss_val = loss.item()

            pb.set_description(
                f"Iteration: true_loss : {loss_val}, scrambled_loss : {scrambled_loss_val} Total Loss : {total_loss_val}"
            )
