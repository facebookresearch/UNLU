...
smnli_label2std_label = defaultdict(
    lambda: "o"
)  # o stands for all other label that is invalid.
smnli_label2std_label.update(
    {"entailment": "e", "neutral": "n", "contradiction": "c", "hidden": "h"}
)

fever_label2std_label = defaultdict(lambda: "o")
fever_label2std_label.update(
    {"SUPPORTS": "e", "NOT ENOUGH INFO": "n", "REFUTES": "c", "hidden": "h"}
)

anli_label2std_label = defaultdict(lambda: "o")
anli_label2std_label.update({"e": "e", "n": "n", "c": "c", "hidden": "h"})
...
def sm_nli2std_format(d_list, filter_invalid=True):
    ...
    for item in d_list:
        ...
        formatted_item["premise_parse"] = item["sentence1_parse"]
        formatted_item["hypothesis_parse"] = item["sentence2_parse"]
        formatted_item["premise_binary_parse"] = item["sentence1_binary_parse"]
        formatted_item["hypothesis_binary_parse"] = item["sentence2_binary_parse"]
...
def ocnli2std_format(d_list, filter_invalid=True, is_test=False):
    p_list: List[Dict] = []
    for item in d_list:
        formatted_item: Dict = dict()
        formatted_item["uid"]: str = item["id"]
        formatted_item["premise"]: str = item["sentence1"]
        formatted_item["hypothesis"]: str = item["sentence2"]
        if not is_test:
            formatted_item["label"]: str = smnli_label2std_label[item["label"]]
            if filter_invalid and formatted_item["label"] == "o":
                continue  # Skip example with invalid label.
